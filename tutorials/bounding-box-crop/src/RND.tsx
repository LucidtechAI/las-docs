...
function normalizeToPixels(dimensions: Dimensions & Position, imageDimensions: Dimensions): Dimensions & Position {
...
function normalizePositionToScale(position: Position, imageDimensions: Dimensions): Position {
  const { width: imageWidth, height: imageHeight } = imageDimensions;
...
function normalizeDimensionsToScale(dimensions: Dimensions, imageDimensions: Dimensions): Dimensions {
  const { width: imageWidth, height: imageHeight } = imageDimensions;
...
function generateSemiRandomId(): string {
  const newBoxId = `newBox-${new Date().getTime()}-${Math.floor(Math.random() * 10000)}`;
...
const RND = ({ doc, predictions, dimensions }: CanvasProps): JSX.Element => {
  ...
  const onChange = (index: number, position?: Position, dimensions?: Dimensions) => {
    setBoundingBoxes((prev) => {
